/******************************************************************************
 * Copyright 2019 The athena Authors. All Rights Reserved.
 *
...
MpcOsqp::MpcOsqp(const Eigen::MatrixXd& matrix_a,
    ...
    : matrix_a_(matrix_a), matrix_b_(matrix_b), matrix_q_(matrix_q),
      ...
      matrix_x_ref_(matrix_x_ref), max_iteration_(max_iter), horizon_(horizon),
      eps_abs_(eps_abs) {
  ...
  std::cout<<"state_dim_:"<<state_dim_<<std::endl;
  std::cout<<"control_dim_:"<<control_dim_<<std::endl;
  std::cout<<"num_param_:"<<num_param_<<std::endl;
}
...
void MpcOsqp::CalculateKernel(std::vector<c_float>* P_data,
  ...
  // TODO(jiang):自定义添加,是否正确?
  if (value_index != num_param_) {
    //AERROR << "value_index != num_param_";
    return;
  }
...
bool MpcOsqp::Solve(std::vector<double>* control_cmd) {
  ...
  if (osqp_setup(&osqp_workspace, data, settings) != 0) {
    //AERROR << "osqp_setup error";
    return false;
  }
  ...
  if (status < 0 || (status != 1 && status != 2)) {
    //ADEBUG << "failed optimization status:\t" << osqp_workspace->info->status;
    osqp_cleanup(osqp_workspace);
    FreeData(data);
    c_free(settings);
    return false;
  } else if (osqp_workspace->solution == nullptr) {
    //ADEBUG << "The solution from OSQP is nullptr";
    osqp_cleanup(osqp_workspace);
    FreeData(data);
    c_free(settings);
    return false;
